<template>
  <div class="my_div">
    ...
    <van-popup
      v-model="showDetail"
      position="top"
      style="height: 65% ; border-radius: 0 0 15px 15px"
    >
      <div>
          ...
          <form>
            ...
            <van-row>
              ...
              <van-button
                ...
                hairline
                type="info"
                v-on:click="modifyAffair(tempAffair.id)"
                v-bind:disabled="dis"
...
export default {
  ...
  methods: {
    ...
    async modifyAffair(id) {
      ...
      (this.dis = true), (this.tempAffair.deadline = this.currentDate);
      if (this.tempAffair.title == "" && this.tempAffair.extra == "") {
        alert("事务和详细说明不能同时为空!");
      } else {
        try {
          const token = localStorage.getItem("token");
